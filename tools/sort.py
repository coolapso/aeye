...
def review_frames(frames_dir: str):
    ...
    for frame in frames:
        ...
        while True:
            ...
            img = cv2.putText(img, texts["exclude"], (0, 115), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,255), 2)
            img = cv2.putText(img, remaining_frames_text, (0, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,255), 2)
...
def main():
    ...
    if len(excluded_frames) > 0:
        print(f'''Moving excluded frames to {dirs["excluded"]}''')
        move_frames(dataset_root_dir, frames_dir, excluded_frames, dirs["excluded"])
